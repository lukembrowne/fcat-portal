...
- ✅ **Phase 2 (portal endpoint) — shipped** on branch `feat/field-upload-endpoint`: `GET /api/field-upload/v1/deployments` (mapping + routing config from the new shared `drive-routing.ts`), timing-safe `FIELD_UPLOAD_TOKEN`, project allow-list, per-IP rate limit, versioned + `minSupportedVersion`. 25 unit tests pass; lint clean; the `drive-client.ts` constant-extraction refactor is behavior-preserving (147 existing tests pass). **Still needed portal-side:** the first-run credential endpoint `GET /api/field-upload/v1/credential` (or Appendix B token-mint).
