...
// Types (DeploymentResult, SnapshotData, SnapshotDelta, AudioReport) and the
// email HTML builder live in `@/lib/nightly-report-email` (pure, testable).
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 7 AM "Actividad del Portal" email). Its totals feed the top dashboard and
    // its detail tables render below. A failure here must NOT block the report.
    let activity: PortalUpdatesPayload | null = null;
    try {
      const windowEnd = new Date();
      const windowStart = new Date(windowEnd.getTime() - 24 * 60 * 60 * 1000);
      activity = await buildPortalUpdatesPayload(windowStart, windowEnd);
    } catch (err) {
...
async function sendReport(
  ...
  const html = buildEmailHtml(date, statusLine, results, delta, errors, audioReport, activity);
